...
class TestGetEc2Region:
    ...
    def az_hop_is_ok(self, mock_requests_get: MagicMock) -> MagicMock:
        """A 200 on the availability-zone request, which the happy paths below assume.

        Same reason as token_is_ok: a bare MagicMock's status_code compares unequal to 200,
        and this hop's status is now checked before its body is used.
        """
    ...
    def token_is_ok(self, mock_requests_put: MagicMock) -> MagicMock:
        """A 200 on the token request, which the happy paths below assume.

        Set explicitly because a bare MagicMock's `status_code` compares unequal to 200, and
        the code now rejects a non-200 token rather than forwarding its body as the token.
        """
    ...
    def test_names_the_cause_when_the_token_is_rejected(
        ...
        """A rejected token must not be forwarded as if it were one.

        Its body is non-empty, so an emptiness check does not catch it. Sent as a token, IMDS
        answers the next hop 401 and the user is told the availability zone was unexpected
        while being shown an error page -- naming the wrong cause.
        """
    ...
    def test_names_the_cause_when_the_az_hop_is_rejected(
        ...
        """Throttling is per-request, so this hop can fail after the token succeeded.

        Unchecked, the error body becomes `az` and is reported as an unexpected availability
        zone -- the same misattribution the token check avoids, one hop later.
        """
    ...
    def test_retries_a_transient_token_status(
        ...
        """5xx belongs here as much as 429 does.

        Boot is when a user-data install runs and when the metadata service is not yet fully
        up, which is the scenario _IMDS_ATTEMPTS exists for -- so treating 500/503 as final
        would leave the retry covering only the narrower half of the transient set.
        """
    ...
    def test_retries_a_lapsed_token_on_the_az_hop(
        ...
        """401 there means the token was rejected, and a fresh one is the fix.

        The 10s TTL requested for the token can lapse between the two calls. Transient on this
        hop only -- a 401 on the token *request* is not re-obtainable.
        """
        mock_requests_get.side_effect = [
    ...
    def test_does_not_retry_a_determinate_answer(
        ...
        """Retrying these would repeat the same message three times, not find a region.

        The user's own case -- a workstation install with no --region -- expects None, so
        multiplying the output makes one failure read as three.
        """
    ...
    def test_does_not_retry_a_black_holed_address(
        ...
        """The workstation case arrives as ConnectTimeout, and must not be retried.

        ConnectTimeout subclasses both ConnectionError and Timeout, so a handler keyed on
        Timeout would make this the retryable case -- five lines of output and ~4s to reach an
        answer already known on the first attempt. This is the case
        test_does_not_retry_a_determinate_answer describes but cannot reach.
        """
    ...
    def test_retries_a_slow_answer_rather_than_failing_the_install(
        ...
        """A bounded request that is not retried would turn a slow EC2 into a failed install.

        `install()` exits 1 when _get_ec2_region returns None and no --region was passed, and
        a user-data-driven install runs during boot, when IMDS is most likely to be slow. So
        the timeout must not be the whole story.
        """
