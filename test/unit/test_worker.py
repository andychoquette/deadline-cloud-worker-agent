...
class TestImdsProbeCost:
    """Pins the cost of the startup IMDS probe, which three constants decide between them.

    Here rather than in a docstring because prose arithmetic rots silently: change any of the
    three values and a comment stating the total goes quietly wrong, whereas this fails. Same
    reason test_monitor_rate pins _EC2_SHUTDOWN_MONITOR_RATE.

    Deliberately *not* a guarantee. `requests` hands the pair to urllib3, whose read timeout is
    an inactivity timeout applied per socket read rather than a ceiling on the read phase, and
    no `total` is set -- so a peer that dribbles a byte just inside the interval can keep one
    attempt going indefinitely. Expressing a true ceiling would mean passing a
    urllib3.Timeout(total=...), which would make this package depend directly on a library it
    only has transitively. What follows is therefore the expected cost against a peer that
    either answers or goes silent, which is every case this probe is written for.
    """
    ...
    def test_a_black_holed_address_costs_less(self) -> None:
        """The common non-EC2 case never reaches the read phase.

        Nothing answers ARP, so each attempt fails in connect. Recorded because the figure above
        overstates what an ordinary workstation or GitHub runner actually pays.
        """
...
class TestRun:
    ...
    def imds_answers(self, requests_put: MagicMock) -> MagicMock:
        """Let the startup IMDS probe succeed on its first attempt.

        Two reasons. Without it the probe exhausts _IMDS_PROBE_ATTEMPTS and pays the real
        backoff twice, adding a second of wall-clock sleep to a test about scheduler
        exception handling. And a negative verdict means run() submits only the scheduler,
        so `submit.side_effect`'s second element is never consumed and the monitor-future
        branch these tests are set up for goes unexercised.
        """
    ...
    def test_a_raising_ec2_probe_shuts_the_scheduler_down(
        ...
        """An exception from the probe must not leave the executor joining a live scheduler.

        The scheduler is already submitted by the time the probe runs, and the enclosing
        `with` calls ThreadPoolExecutor.shutdown(wait=True) on the way out. Without the
        cleanup this test pins, that join never returns and the worker cannot be killed by
        SIGTERM -- the signal handler is not what is blocked. Structural: it holds for any
        exception type, not only the ones the probe's own handler catches.
        """
        # GIVEN
...
class TestIsEc2Host:
    ...
    def test_says_so_when_monitoring_is_disabled(self, worker: Worker) -> None:
        ...
        with (
            ...
            patch.object(worker_mod.logger, "warning") as logger_warning,
        ):
            worker._is_ec2_host()

        logger_warning.assert_called_once()
        assert "not be" in logger_warning.call_args.args[0]


class TestImdsTimeoutIsVisible:
    """A timed-out interruption query must not pass for "nothing is shutting down".

    None from the spot query and False from the ASG query are the caller's "no interruption
    pending", indistinguishable from a real answer. So on a host whose IMDS is reachable but
    slow, the monitor would otherwise report all-clear once a second with no trace that it
    never got an answer -- and a missed spot notice would leave nothing to diagnose.
    """
